	...
	public AssetsHolder(Context context) {
		...
		try {
			...
			int dim = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
					Globals.BULLET_SIZE, context.getResources().getDisplayMetrics());
			plasma = Bitmap.createScaledBitmap(original, dim, dim, false);

			is = manager.open("images/boss.png");
			original = BitmapFactory.decodeStream(is);
			int dimX = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 115, context
					.getResources().getDisplayMetrics());
			int dimY = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 150, context
					.getResources().getDisplayMetrics());
			boss = Bitmap.createScaledBitmap(original, dimX, dimY, false);

			is = manager.open("images/laser1.png");
			original = BitmapFactory.decodeStream(is);
			dim = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, Globals.LASER_SIZE,
			...
			original = BitmapFactory.decodeStream(is);
			mainBackground = Bitmap.createScaledBitmap(original, mSize.x, mSize.y, false);

			is = manager.open("images/stage_background.png");
			original = BitmapFactory.decodeStream(is);
			stageBackground = Bitmap.createScaledBitmap(original, mSize.x, mSize.y, false);
