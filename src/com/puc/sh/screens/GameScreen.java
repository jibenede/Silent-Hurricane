...
public class GameScreen extends Screen {
	...
	private void drawCanvas() {
		...
		if (mState.isAlive()) {
			...
			mCanvas.drawBitmap(mAssets.ship, shipPosition.x, shipPosition.y, null);
		} else {
			if (!death) {
		...
		for (int i = 0; i < enemies.size(); i++) {
			...
			if (!e.isGone()) {
				...
				mMatrix.postRotate(degrees, e.getX() + e.getBitmap().getWidth() / 2, e.getY()
						+ e.getBitmap().getHeight() / 2);
	...
	public boolean onTouchEvent(MotionEvent event) {
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			int shipX = (int) event.getX() - Globals.SHIP_SIZE / 2;
			int shipY = (int) event.getY() - Globals.SHIP_SIZE / 2;
