...
public class Pattern_M extends BulletPattern {
    ...
    private static int BULLET_INTERVAL = 80;
    ...
    public void update(long interval) {
        ...
        if (mTimeUntilNextShot < 0) {
            ...
            mBullet.initializeLinearBullet(mContext.getAssets().yellowBullet24,
                    false, 0, speed, x, 0, 0, 24, 1);
            mContext.getState().mEnemyBullets.addBullet(mBullet);

            x = Utils.sRandom.nextInt(700);
            speed = (int) (BULLET_SPEED + 40 * Utils.sRandom.nextDouble());

            mBullet.initializeLinearBullet(mContext.getAssets().whiteBullet48,
                    false, 0, speed, x, 0, 0, 48, 1);
            mContext.getState().mEnemyBullets.addBullet(mBullet);
