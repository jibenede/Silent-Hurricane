...
public class Stage0 extends Stage {
	...
	public Stage0(AuroraContext context, RenderView renderer) {
		...
		for (int i = 0; i < 3; i++) {
			Raven foe = new Raven(context, 10, 5000, Globals.CANVAS_WIDTH / 2 - i * 80 - 100, 100);
		...
		{
			Raven foe = new Raven(context, 10, 5000, Globals.CANVAS_WIDTH / 2, 100);
			addFoeAtTime(foe, 14000);
		}

		for (int i = 0; i < 7; i++) {
			Volt foe = new Volt(context, 7, i * Globals.CANVAS_WIDTH / 8, 400, -400,
					Volt.Type.VERTICAL);
			addFoeAtTime(foe, 18000 + 500 * i);
		}

		for (int i = 0; i < 3; i++) {
			Volt foe = new Volt(context, 7, 300 + i * 100, 400, -400, Volt.Type.HORIZONTAL_RIGHT);
			addFoeAtTime(foe, 26000 + 1000 * i);
		}

		for (int i = 0; i < 3; i++) {
			Volt foe = new Volt(context, 7, 300 + i * 100, 400, -400, Volt.Type.HORIZONTAL_LEFT);
			addFoeAtTime(foe, 26000 + 1000 * i);
		}

		{
			Angel foe = new Angel(context, 100);
			foe.setPositions(Globals.CANVAS_WIDTH / 2 - foe.mBitmap.getWidth() / 2,
					-foe.mBitmap.getHeight(),
					Globals.CANVAS_WIDTH / 2 - foe.mBitmap.getWidth() / 2, 250);
	...
	public Audio getStageTheme() {
		return mContext.getAssets().stage0_audio;
	}

	@Override
	public Audio getBossTheme() {
		return mContext.getAssets().boss0_theme;
	}
