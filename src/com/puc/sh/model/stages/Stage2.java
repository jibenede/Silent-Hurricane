...
public class Stage2 extends Stage {
	...
	public Stage2(AuroraContext context, RenderView renderer) {
		...
		for (int i = 0; i < positionsX.length; i++) {
			...
			if (positionsX[i] < 360) {
				foe.setPositions(positionsX[i] - foe.mBitmap.getWidth() / 2 - 200,
						-foe.mBitmap.getHeight() - 100, positionsX[i] - foe.mBitmap.getWidth() / 2,
						positionsY[i]);
			} else {
				foe.setPositions(positionsX[i] + foe.mBitmap.getWidth() / 2 + 200,
						-foe.mBitmap.getHeight() - 100, positionsX[i] - foe.mBitmap.getWidth() / 2,
		...
		for (int i = 0; i < 30; i++) {
			Eclipse foe1 = new Eclipse(context, 12, Eclipse.Direction.RIGHT, 100);
		...
		for (int i = 0; i < 14; i++) {
			...
			Stingray foe4 = new Stingray(context, 10, 280, Stingray.Direction.LEFT);
